...
contract CustomerAccount {
    ...
    // We need to pass "String" as in memory.
    ...
    // if the contract needs to recieve money; then the function needs to be public
    //Note here that the deposit is NOT a Keyword.
    function deposit() public payable {
        accountBalance = address(this).balance;
    }
    // Here we want the address "variable" should be payable
    // amount needs to be uint
    // funciton needs to be set to either private or public (we also have external and internal)
    function sendRemittance(address payable recipient, uint amount) public {
        recipient.transfer(amount);
        accountBalance = address(this).balance;        
    }
